...
const ContentDirectories: React.FC<{ classActive: string }> = ({ classActive }) => {
  ...
  const createNewDirectoryHandler = (inputValue: string) => {
    ...
    const directoryDoesNotExist = directories.every((dir: string) => dir !== newDirectoryName);
  ...
  return (
    <>
      {modalDirIsShown && (
        <ModalDirectory onClose={closeModalDirectoryHandler} onConfirm={createNewDirectoryHandler} btnText="Create" title="Create new directory" />
      )}
