...
const classHasNotification =
  "after:content-[''] after:w-2 after:h-2 after:bg-amber-500 block after:rounded-full after:absolute after:bottom-3/4  after:left-3/4";
      ...
      {notificationIsVisible && (
        <div className="absolute bg-slate-100 dark:bg-slate-800 top-full rounded-md right-0 p-3 w-52 border border-slate-300 dark:border-slate-700">
          {uncompletedTasks.length > 0 ? (
            <div>
              <span className="dark:text-slate-200 font-medium">You have {uncompletedTasks.length} uncompleted tasks today:</span>
              <ul>
              ...
              {moreTasksToShow && (
                <a
                  href="/"
                  className="transition block w-full rounded-md p-1 bg-amber-100 text-amber-600 dark:text-slate-200 dark:bg-slate-700/[.3] text-center"
                >
                  See today's tasks
                </a>
              )}
