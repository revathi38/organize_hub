    ...
    <>
      <li key={task.id}>
        <Link
          to={`/dir/${task.dir}`}
          title={task.dir}
          className="ml-auto mr-4 w-min whitespace-nowrap overflow-hidden max-w-[10rem] text-center text-ellipsis bg-amber-200 text-amber-600 px-4 py-1 rounded-t-md transition dark:bg-slate-700 dark:text-slate-200 block hover:bg-amber-300 dark:hover:bg-amber-500"
        >
          {task.dir}
        </Link>
        <article
